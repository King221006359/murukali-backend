...
// TEMPORARY DEBUG: log every single incoming request, unconditionally,
// before any route handler runs. This lets us see exactly which
// request triggers a crash, even if it happens before reaching
// the route-specific try/catch block.
app.use((req, res, next) => {
  console.log(`>>> INCOMING ${req.method} ${req.originalUrl} | body: ${JSON.stringify(req.body)}`);
  next();
});

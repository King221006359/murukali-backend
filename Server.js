...
// (momo, cash, fix_momo, cred_dpo, amount, qty, cost_unit, etc. are all
// NUMERIC in schema.sql). Without this, frontend math like
// `obMomo + momoIn - momoOut` silently does STRING CONCATENATION instead
// of addition whenever the left-hand value came straight from the DB
// (e.g. "5000" + 0 → "50000" instead of 5000). This tells the driver to
// parse OID 1700 (numeric) as a JS float instead, fixing it everywhere
// at once.
...
const pool = new Pool({
  ...
  // Neon's pooled (-pooler) endpoint routes through PgBouncer in
  // transaction pooling mode, which does not preserve prepared
  // statements across queries (a Parse frame from one query can be
  // "forgotten" by the next Bind, since each transaction may land on
  // a different physical server connection). This produces errors
  // like "bind message supplies N parameters, but prepared statement
  // '' requires 0". The documented fix for node-postgres is to
  // disable prepared statements entirely on this pool.
  prepare: false,
});

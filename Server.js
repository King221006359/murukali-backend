...
const pool = new Pool({
  ...
  // "transaction" pooling mode. PgBouncer in that mode does not
  // support named/cached prepared statements shared across queries
  // on the same physical connection — but node-postgres's default
  // Client implementation still negotiates the extended query
  // protocol (Parse/Bind/Execute) per query, which can desync once
  // PgBouncer hands the backend connection to a different client
  // mid-session. Setting max:1 forces this pool to hold a single
  // dedicated connection for this whole process instead of cycling
  // through PgBouncer's shared pool for every query, which avoids
  // the statement-cache desync entirely for a small backend like
  // this one.
  max: 1,
